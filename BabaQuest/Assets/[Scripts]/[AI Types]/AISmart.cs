    ...
    class AISmart : MonoBehaviour, AITypeInterface
    {
        ...
        private void CheckAttack(CharacterTypeInterface character, bool[] good, int[] possitions, int movesLeft, int targetNr, int myNr)
        {
            ...
            if (character.GetType().Name != "Mage")
            {
                ...
                if (movesLeft == 2) // just healed, do not have full life yet
                {
                    ...
                    if (Math.Abs(targetPos - myNewPos) <= myReach) // if i can reach it, then attack and go away if I can
                    {
                        ...
                        {
                            if (good[possitions[myNewPos - ((targetPos - myNewPos) / Math.Abs(targetPos - myNewPos)) * myWalk]] != good[myNr]
                    ...
                    else if (Math.Abs(targetPos - (myNewPos + ((targetPos - myNewPos) / Math.Abs(targetPos - myNewPos)) * myWalk)) <= myReach) // if I can go near him and hit him
                    {
                        ...
                        {
                            if (good[possitions[myNewPos + ((targetPos - myNewPos) / Math.Abs(targetPos - myNewPos)) * myWalk]] != good[myNr]) // can I go there?
                            {
                                walk--;
                                if (walk == 0) // I cannot neither walk, nor attack, I just heal
                                {
            ...
            else // walk for mage
            {
                ...
                {
                    if (good[i] != good[myNr] && i == 0) // is it against me and is he standing on this tile?
                    {
                        t0 = false;
                    }
                    if (good[i] != good[myNr] && i == 3) // is it against me and is he standing on this tile?
                    {
                        t3 = false;
                    }
                    if (good[i] != good[myNr] && i == 6) // is it against me and is he standing on this tile?
                    {
                        t6 = false;
                    }
                }
